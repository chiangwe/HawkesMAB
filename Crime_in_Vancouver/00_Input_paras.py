...
#tol_rounds = 500 
#rounds_tr = 50
tol_rounds = int(sys.argv[1].split('=')[1])
rounds_tr = int(sys.argv[2].split('=')[1])

# Read processed csv 
df = pd.read_csv('../raw_data/Crime_in_Vancouver/crime_processed.csv')

# Sort date time
df = df.sort_values(by="datetime_occur").reset_index(drop=True)

# Get reciever code and time
df = df[['crime_type', 'datetime_occur']]
...
df_sel['datetime_occur'] = pd.to_datetime( df_sel['datetime_occur'] ).dt.round('1s')
df_sel['unix_s'] = df_sel['datetime_occur'].apply(lambda x: x.timestamp()/60 )
