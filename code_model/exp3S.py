...
def exp3S(job_sim, method, sim_phase):
	...
	while ( True ):
		try:
			...
			itr_K, itr_sim, gamma, alpha = job_sim.get_nowait()
			itr_K = int(itr_K)
			...
			if(sim_phase == "val_phase"):
				sim_path_out = './' + save_dir + '/' + method + '/'+ 'K_'+ "{:02d}".format(int(itr_K)) + '_sim_'  + "{:04d}".format(int(itr_sim)) +\
							   "_gamma_" + "{:04f}".format(float(gamma)) + "_alpha_" + "{:04f}".format(float(alpha)) + '.npz'
			elif(sim_phase == "te_phase"):
				sim_path_out = './' + save_dir + '/' + method + '/'+ 'K_'+ "{:02d}".format(int(itr_K)) + '_sim_' + "{:04d}".format(int(itr_sim)) +\
							   "_gamma_" + "{:04f}".format(float(gamma)) + "_alpha_" + "{:04f}".format(float(alpha)) + '.npz'
			...
			for rounds in range(0, all_rounds):
				...
				try:
					...
					print( sim_path_out )
				#a_sel = np.random.choice(Ktol, 1, p=probs)[0]
				#try:
				#	a_sel = np.random.choice(Ktol, 1, p=probs)[0]
				#except:
				#	pdb.set_trace()
				...
				#if np.isinf( np.multiply( exp_x_hat, w_k ) + np.exp(1)*alpha*w_k.sum()/Ktol ).any():
				#	pdb.set_trace()
				w_k = np.multiply( exp_x_hat, w_k ) + np.exp(1)*alpha*w_k.sum()/Ktol
